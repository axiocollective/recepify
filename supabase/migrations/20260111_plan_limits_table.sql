...
insert into public.plan_action_limits (plan, imports, translations, optimizations, ai_messages)
values
  ('base', 0, 0, 0, 0),
  ('premium', 25, 25, 25, 150)
on conflict (plan) do update
set imports = excluded.imports,
...
declare
  ...
  v_plan_imports integer;
  v_plan_translations integer;
  v_plan_optimizations integer;
  v_plan_ai_messages integer;
  v_period_start date;
  v_usage_id uuid;
...
begin
  ...
  select imports, translations, optimizations, ai_messages
  into v_plan_imports, v_plan_translations, v_plan_optimizations, v_plan_ai_messages
  from public.plan_action_limits
  where plan = v_profile.plan;

  v_plan_imports := coalesce(v_plan_imports, 0);
  v_plan_translations := coalesce(v_plan_translations, 0);
  v_plan_optimizations := coalesce(v_plan_optimizations, 0);
  v_plan_ai_messages := coalesce(v_plan_ai_messages, 0);

  if action_type = 'import' then
    v_plan_limit := v_plan_imports;
    ...
    v_plan_limit := v_plan_translations;
    ...
    v_plan_limit := v_plan_optimizations;
    ...
    v_plan_limit := v_plan_ai_messages;
